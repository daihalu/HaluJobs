<template>
  <div @drop="_drop" @dragenter="_suppress" @dragover="_suppress" style="margin-top: 100px">
    ...
    <div class="row">
      <div class="col-xs-12">
        <button :disabled="excelData.length ? false : true" class="btn btn-success" @click="_exportToExcel">Export
        </button>
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      _drop(evt) {
        evt.stopPropagation();
        evt.preventDefault();
        const files = evt.exelDataTransfer.files;
      ...
      _readFile(file) {
        ...
        reader.onload = (e) => {
          ...
          for (let i = 0; i < this.workSheetNames.length; i++) {
            ...
            let exelData = XLSX.utils.sheet_to_json(ws, {header: 1});
            // console.log(JSON.stringify(exelData));
